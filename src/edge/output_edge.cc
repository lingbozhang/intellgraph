...
template <class T>
OutputEdge<T>::OutputEdge(const struct EdgeParameter<T>& edge_param)
...
template <class T>
void OutputEdge<T>::PrintWeight() const {
  std::cout << "OutputEdge " << " Weight matrix:" << std::endl 
            << weight_ptr_->array() << std::endl;
}

template <class T>
void OutputEdge<T>::PrintNablaWeight() const {
  std::cout << "OutputEdge " << " Nabla weight matrix:" << std::endl 
            << nabla_weight_ptr_->array() << std::endl;    
}
...
template <class T>
void OutputEdge<T>::ApplyUnaryFunctor(std::function<T(T)> functor) {
  if (functor == nullptr) {
...
template <class T>
MatXXSPtr<T> OutputEdge<T>::GetWeightPtr() {
  return weight_ptr_;
}

// Instantiate class, otherwise compilation will fail
template class OutputEdge<float>;
template class OutputEdge<double>;
