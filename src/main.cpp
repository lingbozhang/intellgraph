...
int main(int argc, char* argv[]) {
  ...
  google::InitGoogleLogging(argv[0]);
       ...
       size_t nbr_small_training_data = 1000;
       MatXX<float> small_training_images = training_images.block(0, 0, 784, \
           nbr_small_training_data);
       MatXX<float> small_training_labels = training_labels.block(0, 0, 10, \
           nbr_small_training_data);
 ...
 // SigmoidNode is an input layer which uses Sigmoid function as activation
       ...
       PermutationMatrix<Dynamic, Dynamic> perm(nbr_small_training_data);
       ...
       std::shuffle(perm.indices().data(), perm.indices().data() + \
              perm.indices().size(), e);
       training_images = small_training_images * perm;
       training_labels = small_training_labels * perm;
       for (int i = 0; i < nbr_small_training_data - minbatch_size; i += minbatch_size) {
       classifier.Backward(small_training_images.block(0, i, 784, minbatch_size), \
                           small_training_labels.block(0, i, 10, minbatch_size));
       // Stochastic gradient decent + L2 regularization
       classifier.get_edge_weight_ptr(1, 4)->array() = \
              (1.0 - eta * lambda / nbr_small_training_data) * \
              classifier.get_edge_weight_ptr(1, 4)->array() - \
